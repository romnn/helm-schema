...
## Short-circuit-flag round (2026-07-25, twenty-ninth round)

F99 (grafana's finite traversal does not constrain intermediate hosts)
closed. The walk is
`and $shouldContinue (hasKey $currentMap $elem)` inside a range over one
sensitive path's segments, with `$currentMap` advancing one member per
iteration; each iteration's `hasKey` requires its subject to be a map,
which the corpus accepted violations of at every host below
`grafana.ini`.

### The defect: an approximate stand-in for a decidable operand

`eval_short_circuit_args` derives each operand's truthiness from its
VALUE: a direct raw identity path becomes `Predicate::truthy_path`, and
everything else becomes `Predicate::approximate`. A boolean flag local
has no values-path identity (a `true` literal evaluates to no
`AbstractValue` at all), so `$shouldContinue` fell into the approximate
arm — and that conjunct rides every capture the operand guards, both as
the later operands' execution guard and, downstream, as an undecodable
conjunct on the `hasKey` subject's `ValueType` capture. The claim then
either abstained or landed unconditionally, which is how the same walk
managed to false-reject the config helper's non-map sections while
missing the sensitive ones.

The flag's truthiness was already in the model, one representation away:
`SymbolicLocalState::truthy_reductions` binds the RHS condition
predicate on declaration (`$shouldContinue := true` is a faithful
`Predicate::True`). What was missing was a channel — `EvalEnv` carried
`locals` (`AbstractValue`s) but no reduction view.

### The fix: a statically-true `and` operand contributes no guard

`EvalEnv::local_truthy_reductions` is populated where `eval_hole_exprs`
builds the header environment, and an `and` operand that is statically
TRUE (a `true` literal, or a local whose reduction is `Predicate::True`)
now contributes NO execution predicate instead of an approximate one:
the later operands execute exactly where the ambient predicates already
say.

Only that polarity is sound. A reduction is a SUFFICIENT condition for
truthiness, so it may stand in for an `and`'s positive execution guard
(narrowing at worst) but never for an `or`'s negated one, which would
widen the guard into states the operands never run in. Every other
combination keeps the approximate stand-in, unchanged.

### The join is not exact, and does not need to be

The previous round's open question was whether the branch join leaves
iteration K's flag an exact conjunction. It does not:
`join_predicate_union` returns `Predicate::True` as soon as one arm's
reduction is True, and the then-arm never reassigns the flag, so the
`$shouldContinue = false` kill switch is unioned away and iteration K's
flag reads as unconditionally truthy.

That is still exact for this claim shape. The requirement recorded at
iteration K is "the advanced host is a map", which only bites where that
host is PRESENT — and its presence is exactly what made the earlier
iterations' `hasKey` hold and left the flag alive. Making the join
exact (stamping unchanged arm reductions with their arm conditions)
would produce the same acceptance set behind a `hasKey`-shaped guard,
at the cost of changing reduction semantics for every accumulator in the
corpus; the cheaper claim is the one whose scope already implies the
dropped conjunct.

### Collateral: the advance stays a single path

With the operand exact, the arm CONDITION decodes exactly too, so
`activate_if` pushes the member's presence guard and the guarded
self-advance is marked (`traversal_advances`). `$currentMap` is then one
path per iteration instead of a join CHOICE of `{grafana.ini,
grafana.ini.database}` — which removes the mis-attributed leaf claim
that used to add a `string` alternative to the host's union base and let
`grafana.ini.database: "text"` validate.

### Adjudication

Two fixtures move: grafana and kube-prometheus-stack (its vendored
grafana subchart), 20 new conditional arms each. Both regenerate from
one clean dump run of the final build.

- 60 new rejections, from the 20 sensitive sections × `7`/`"text"`/`[]`:
  every one ABORTS under `helm template --skip-schema-validation` with
  "wrong type for value; expected map[string]interface {}" at
  `_helpers.tpl:260`. `{}` renders and still validates, and the
  non-sensitive control section (`grafana.ini.customSection: 7`) stays
  open in both.
- 33 probe flips in the LOOSENING direction, all at
  `grafana.ini.{analytics,log,server}` and their leaves: every one
  RENDERS. `_config.tpl` handles non-map sections explicitly
  (`if not (kindIs "map" $elemVal)` with per-kind arms), so those
  rejections were false — the old unconditional claim.
- Zero regressions: the old-versus-new battery over 408 grafana and
  1541 kube-prometheus-stack probe paths (every path in the composed
  defaults × int/str/bool/empty-list/list/empty-map/member-map/deletion)
  reports no tightening that helm renders, and the whole unit and
  integration suites are otherwise byte-identical.
- The new schema agrees with helm on 82/82 sensitive-host probes
  (the old one mismatched 60) and on the kube-prometheus-stack spot
  checks including the subchart-disabled dormant state.

Pinned by
`statically_true_flag_operands_keep_short_circuit_operand_contracts`
and `unrolled_traversal_binds_every_intermediate_host_kind` (gen: the
minimal flag operand and the full unrolled walk, each with its dormant
sibling) plus `grafana_leaked_secret_walk_requires_map_hosts` (CLI:
int/string/list hosts, the atomic dotted-key host, the empty host, the
expanded leaf, and the dormant assertion).

### Validation

`task test` green (918 unit tests), `task test:integration` green (488,
including every chart pin), `task lint` clean (clippy, ast-grep, fmt),
corpus fixtures byte-identical to one clean dump run of the final build,
and the luup2 `check:local` downstream gate passes with the installed
