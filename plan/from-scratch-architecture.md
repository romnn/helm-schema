...
- **A1 — finish interpreter phases 3–4 with the corrected shapes**:
  ...
  current `ValueUse` boundary. Scope snapshots now sit behind one
  `SymbolicScopeState` boundary that owns predicates, current-dot stack, and
  local bindings, so node evaluation no longer knows about compatibility
  guard/dot-stack mechanics. The
...
- **A2 — helper summaries under the §6.3 contract (complete)**: empty-pc summaries
  ...
  compatibility state. Helper value-fact collection and structured helper
  fragment output-use collection now use the shared tree-sitter node evaluator
  too, so helper-body traversal no longer has separate source-order
  interpreters for those facts. Partial scalar interpolation is represented as
  `ValueKind::PartialScalar` at the compatibility DTO boundary and lowers to
  weak string-render evidence only when no stronger schema signal exists. The
...
- **A4 — `ContractIR` + resolution/lowering (complete)**: the
  ...
  compatibility projection policy for ambient guards, render-suppressed paths,
...
- **B1 — planner/executor (complete)**: pure `plan()` + one `execute()` +
  ...
  chain decides the miss is final. Provider lookup memoization now lives
...
- **B3 — capability oracle adapter** + `kube_version()`; `ProbeTable` as
  ...
  now parse into an IR-owned `ApiPresenceQuery`, where resource-qualified
  checks carry only `{ apiVersion, kind }` rather than a full schema-resolution
