...
## Intake: 100 charts adopted, unadjudicated

The 100-chart intake is now vendored, against the adoption ordering above but
ahead of the fixes — deliberately, so the correctness sweep has something to
sweep. Composition:

- **24 quarantined false rejections** — the confirmed defects, listed in
  `QUARANTINED_FALSE_REJECTIONS` in `crates/helm-schema-cli/tests/chart_corpus.rs`.
  Their tests assert that self-validation **fails**, so each defect is pinned and
  visible; when a fix lands the test breaks and the chart must be promoted out of
  the list together with an adjudicated fixture.
- **76 charts that currently pass**, chosen by popularity among those generating
  in under 10 s, to keep the integration suite's runtime bounded.

All 100 are listed in `UNADJUDICATED_INTAKE`. Their fixtures pin **current
output, not correct output** — a diff against one means "output changed", never
"output regressed". A consistency test keeps the two rosters from drifting apart.

Two facts established by the intake itself:

- **The vendored provider bundle grew from 83 to 158 Kubernetes schemas and 27 to
  105 CRD schemas, and all 56 pre-existing fixtures regenerated byte-identical.**
  The merge is purely additive; no existing chart silently gained or lost
  provider-backed facts.
- **18 of the 156 corpus schemas now exceed 5 MiB**, which is Helm's own
  chart-file limit — they could not ship as a chart's `values.schema.json` at all.
  The largest is openebs at 33 MB. That is not merely a size problem: openebs,
  milvus, oncall, gitea and redmine are all D3 charts, and cross-chart leakage is
  precisely what inflates them. Schema size is therefore a cheap proxy metric for
  the leakage fix, and should shrink sharply when D3 lands.
