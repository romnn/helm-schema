...
impl ResourceCursor {
    ...
    pub(crate) fn from_source(source: &str, defines: &DefineIndex) -> Self {
        let mut spans = document_spans(source)
            .into_iter()
            .map(|(start, end)| {
                let resource = source.get(start..end).and_then(|doc_src| {
                    TreeSitterParser
                        .parse(doc_src)
                        .ok()
    ...
    pub(crate) fn advance_to(&mut self, byte: usize) {
        self.current_span = self
            .spans
            .iter()
            .position(|span| span.start <= byte && byte < span.end);
    }
    ...
    pub(crate) fn current(&self) -> Option<ResourceRef> {
        self.current_span
            ...
            .and_then(|span| span.resource.clone())
    }
}
