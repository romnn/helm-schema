...
//! target Kubernetes cluster supports. The IR layer decodes that structural
//! shape into `HelperBranch { guard, body }` entries, and this module decides
//! which branches can be live for a caller-provided capability oracle.
//!
...
//!
//! Module layout: single-responsibility split so this stays a small
//! interpreter as more guard shapes are added:
//!   - [`CapabilityOracle`] trait: pluggable "is api X supported in
//!     the target K8s version" oracle. The knowledge layer provides
//!     the production implementation; tests can supply a [`StaticOracle`]
//!     with explicit yes/no answers.
...
/// a specific Kubernetes version. Production implementation lives in the
/// knowledge layer and consults the configured providers; tests use
...
/// across delegation depth. Branches with empty bodies (no literal
/// reachable, even through nesting) are skipped.
///
...
mod tests {
    ...
    fn select_picks_if_branch_when_capability_unknown() {
        // Unknown capability is treated as potentially-live, so the if-branch
    ...
    fn select_handles_elif_chain_picking_middle_branch() {
        ...
        // guard evaluates true, so that branch is live.
    ...
    /// An outer if/else where the if-branch delegates to a branched inner helper:
