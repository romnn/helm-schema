...
impl ContractPathAccumulator {
    ...
    pub(super) fn into_schema_evidence(
        ...
        // Rows inside a range may add guards beyond the header's own set.
        // Inherit only when the row predicate exactly implies the range
        // predicate, so complementary siblings cannot lend their runtime
        // domain across the partition.
