...
use helm_schema_mapper::schema::{generate_values_schema_vyt, DefaultVytSchemaProvider, UpstreamK8sSchemaProvider, UpstreamThenDefaultVytSchemaProvider};
...
#[test]
fn snapshot_full_fixture_ir_and_schema() -> eyre::Result<()> {
    Builder::default().build();

    let root = VfsPath::new(vfs::PhysicalFS::new(env!("CARGO_MANIFEST_DIR")));
    let chart_dir = fixture_root(&root, "full-fixture")?;
    ...
    let ir_path = snaps_dir.join("full-fixture.ir.json");
    let schema_path = snaps_dir.join("full-fixture.values.schema.json");
