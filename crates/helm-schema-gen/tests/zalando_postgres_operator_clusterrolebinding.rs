...
const TEMPLATE_PATH: &str = "charts/zalando-postgres-operator/templates/clusterrolebinding.yaml";
...
fn schema_fused_rust() {
    ...
    if std::env::var("SCHEMA_DUMP").is_ok() {
        ...
        let path = std::env::temp_dir()
            .join("helm-schema.zalando-postgres-operator.clusterrolebinding.schema.json");
        std::fs::write(
            &path,
