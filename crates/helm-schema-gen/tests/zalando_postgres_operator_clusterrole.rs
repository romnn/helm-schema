...
const TEMPLATE_PATH: &str = "charts/zalando-postgres-operator/templates/clusterrole.yaml";
...
fn schema_fused_rust() {
    ...
    if std::env::var("SCHEMA_DUMP").is_ok() {
        ...
        let path = std::env::temp_dir()
            .join("helm-schema.zalando-postgres-operator.clusterrole.schema.json");
        std::fs::write(
            &path,
    ...
    let expected: serde_json::Value = serde_json::from_str(include_str!(
        "fixtures/zalando_postgres_operator_clusterrole.schema.json"
    ))
