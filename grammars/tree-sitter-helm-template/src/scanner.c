...
typedef enum {
    ...
    R_HLM_TPL, BR_HLM_TPL, B_HLM_TPL,
    R_HLM_DIR, BR_HLM_DIR, B_HLM_DIR,
...
#define RET_SYM(RESULT_SYMBOL)                                                                                         \
    {                                                                                                                  \
        flush(scanner);                                                                                                \
        ts_dbg_ret(scanner->row, scanner->col, RESULT_SYMBOL);                                                         \
        lexer->result_symbol = RESULT_SYMBOL;                                                                          \
        return true;                                                                                                   \
    }

static inline const char *ts_dbg_tok_name(TokenType t) {
    switch (t) {
    case END_OF_FILE:
        return "END_OF_FILE";
    case R_BLK_VAL_BGN:
        return "R_BLK_VAL_BGN";
    case BR_BLK_VAL_BGN:
        return "BR_BLK_VAL_BGN";
    case B_BLK_VAL_BGN:
        return "B_BLK_VAL_BGN";
    case R_BLK_IMP_BGN:
        return "R_BLK_IMP_BGN";
    case BL:
        return "BL";
    case COMMENT:
        return "COMMENT";
...
static inline bool ts_dbg_enabled(void) {
    static int inited = 0;
    static bool enabled = false;
    if (!inited) {
        enabled = getenv("HELM_SCHEMA_TS_DEBUG") != NULL;
        inited = 1;
    }
    return enabled;
}

static inline bool ts_dbg_row_ok(int row) {
    static int inited = 0;
    static long min_row = -1;
    static long max_row = -1;
    if (!inited) {
        const char *min_s = getenv("HELM_SCHEMA_TS_DEBUG_ROW_MIN");
        const char *max_s = getenv("HELM_SCHEMA_TS_DEBUG_ROW_MAX");
        if (min_s)
            min_row = strtol(min_s, NULL, 10);
        if (max_s)
            max_row = strtol(max_s, NULL, 10);
        inited = 1;
    }
    if (min_row >= 0 && (long)row < min_row)
        return false;
    if (max_row >= 0 && (long)row > max_row)
        return false;
    return true;
}

static inline void ts_dbg_ret(int row, int col, TokenType t) {
    if (!ts_dbg_enabled())
        return;
    if (!ts_dbg_row_ok(row))
        return;
    const char *nm = ts_dbg_tok_name(t);
    if (nm) {
        fprintf(stderr, "ts-scn ret %s at %d:%d\n", nm, row + 1, col + 1);
    } else {
        fprintf(stderr, "ts-scn ret %d at %d:%d\n", (int)t, row + 1, col + 1);
    }
}
...
static unsigned serialize(Scanner *scanner, char *buffer) {
    ...
    *(int8_t *)&buffer[size] = scanner->skip_empty_map_after_else;
    size += sizeof(int8_t);
    *(int16_t *)&buffer[size] = scanner->skip_empty_map_after_else_col;
    size += sizeof(int16_t);
...
static void deserialize(Scanner *scanner, const char *buffer, unsigned length) {
    ...
    if (length > 0) {
        ...
        if (size + sizeof(int16_t) + sizeof(int8_t) + sizeof(int16_t) <= length) {
            ...
            size += sizeof(int16_t);
            scanner->skip_empty_map_after_else = *(int8_t *)&buffer[size];
            size += sizeof(int8_t);
            scanner->skip_empty_map_after_else_col = *(int16_t *)&buffer[size];
            size += sizeof(int16_t);
        }
        while (size < length) {
...
static bool scan(Scanner *scanner, TSLexer *lexer, const bool *valid_symbols) {
    ...
    if (scanner->cur_row > scanner->row && scanner->blk_map_colon >= 0 && scanner->cur_col < scanner->blk_map_colon) {
        scanner->blk_map_colon = -1;
    }

    if (scanner->skip_empty_map_after_else) {
        if (allow_comment && valid_symbols[COMMENT]) {
            bool has_nwl_pre = scanner->cur_row > scanner->row;
            bool is_line_start_pre = has_nwl_pre || scanner->cur_col == 0;
            if (is_line_start_pre && scanner->cur_col > scanner->skip_empty_map_after_else_col && lexer->lookahead == '{') {
                adv(scanner, lexer);
                if (lexer->lookahead == '}') {
                    adv(scanner, lexer);
                    while (is_wsp(lexer->lookahead)) {
                    ...
                    if (is_nwl(lexer->lookahead) || lexer->lookahead == 0) {
                        mrk_end(scanner, lexer);
                        scanner->skip_empty_map_after_else = 0;
                        scanner->skip_empty_map_after_else_col = 0;
                        RET_SYM(COMMENT);
    ...
    bool has_nwl_pre = scanner->cur_row > scanner->row;
    bool is_line_start_pre = has_nwl_pre || bgn_col == 0;

    if (valid_symbols[BL] && bgn_col <= cur_ind && !has_tab_ind) {
        bool should_pop = cur_ind == prt_ind && cur_ind_typ == IND_SEQ ? bgn_col < cur_ind || lexer->lookahead != '-'
                                                                        : bgn_col <= prt_ind || cur_ind_typ == IND_STR;
        if (should_pop) {
            bool can_hlm = allow_comment &&
                           (valid_symbols[R_HLM_DIR] || valid_symbols[BR_HLM_DIR] || valid_symbols[B_HLM_DIR] ||
 ...
 no_bl:
    ...
    if (lexer->lookahead == '%') {
        ...
        if (valid_symbols[B_FLW_SEQ_END] && is_b) {
            adv(scanner, lexer);
            mrk_end(scanner, lexer);
            RET_SYM(B_FLW_SEQ_END)
        }
    } else if (lexer->lookahead == '{') {
        bool can_r_hlm_tpl = valid_symbols[R_HLM_TPL];
        bool can_br_hlm_tpl = valid_symbols[BR_HLM_TPL];
        bool can_b_hlm_tpl = valid_symbols[B_HLM_TPL];
        bool is_r_hlm_tpl = can_r_hlm_tpl && is_r;
        bool is_br_hlm_tpl = can_br_hlm_tpl && is_br;
        bool is_b_hlm_tpl = can_b_hlm_tpl && is_b;

        bool can_r_hlm_dir = valid_symbols[R_HLM_DIR];
        bool can_br_hlm_dir = valid_symbols[BR_HLM_DIR];
        bool can_b_hlm_dir = valid_symbols[B_HLM_DIR];
        bool is_r_hlm_dir = can_r_hlm_dir && is_r;
        bool is_br_hlm_dir = can_br_hlm_dir && is_br;
        bool is_b_hlm_dir = can_b_hlm_dir && is_b;

        bool can_comment = allow_comment && valid_symbols[COMMENT];
        bool can_hlm_tpl = can_r_hlm_tpl || can_br_hlm_tpl || can_b_hlm_tpl;
        bool can_hlm_dir = can_r_hlm_dir || can_br_hlm_dir || can_b_hlm_dir;

        bool is_r_flw_map_bgn = valid_symbols[R_FLW_MAP_BGN] && is_r;
        bool is_br_flw_map_bgn = valid_symbols[BR_FLW_MAP_BGN] && is_br;
        bool is_b_flw_map_bgn = valid_symbols[B_FLW_MAP_BGN] && is_b;

        if (can_hlm_tpl || can_hlm_dir || can_comment || is_r_flw_map_bgn || is_br_flw_map_bgn || is_b_flw_map_bgn) {
            adv(scanner, lexer);
            bool is_line_start = has_nwl || bgn_col == 0;
            bool prefer_hlm_dir = is_line_start && can_hlm_dir;
            bool prefer_hlm_tpl = can_hlm_tpl && !prefer_hlm_dir;

            if ((can_hlm_tpl || can_hlm_dir || can_comment) && lexer->lookahead == '{') {
                adv(scanner, lexer);
                bool is_hlm_blk_comment = false;
                bool is_hlm_control = false;
                int32_t hlm_first_expr_chr = 0;
                bool is_hlm_else = false;
                ...
                hlm_first_expr_chr = lexer->lookahead;

                {
                    char kw[8];
                    unsigned k = 0;
                    while (k + 1 < sizeof(kw) && ((lexer->lookahead >= 'a' && lexer->lookahead <= 'z') ||
                                                  (lexer->lookahead >= 'A' && lexer->lookahead <= 'Z'))) {
                        kw[k++] = (char)lexer->lookahead;
                        adv(scanner, lexer);
                    }
                    kw[k] = '\0';
                    if ((k == 2 && kw[0] == 'i' && kw[1] == 'f') || (k == 3 && kw[0] == 'e' && kw[1] == 'n' && kw[2] == 'd') ||
                        (k == 4 && kw[0] == 'e' && kw[1] == 'l' && kw[2] == 's' && kw[3] == 'e') ||
                        (k == 4 && kw[0] == 'w' && kw[1] == 'i' && kw[2] == 't' && kw[3] == 'h') ||
                        (k == 5 && kw[0] == 'r' && kw[1] == 'a' && kw[2] == 'n' && kw[3] == 'g' && kw[4] == 'e') ||
                        (k == 6 && kw[0] == 'd' && kw[1] == 'e' && kw[2] == 'f' && kw[3] == 'i' && kw[4] == 'n' && kw[5] == 'e') ||
                        (k == 5 && kw[0] == 'b' && kw[1] == 'l' && kw[2] == 'o' && kw[3] == 'c' && kw[4] == 'k')) {
                        is_hlm_control = true;
                    }
                    if (k == 4 && kw[0] == 'e' && kw[1] == 'l' && kw[2] == 's' && kw[3] == 'e') {
                        is_hlm_else = true;
                    }
                }
                ...
                for (;;) {
                    ...
                    if (lexer->lookahead == '}') {
                        ...
                        if (lexer->lookahead == '}') {
                            ...
                            if (is_hlm_control && is_line_start && can_comment) {
                                while (lexer->lookahead != 0 && !is_nwl(lexer->lookahead)) {
                                    adv(scanner, lexer);
                                }
                                mrk_end(scanner, lexer);
                                RET_SYM(COMMENT);
                            ...
                            if (treat_as_tpl && is_concat_after) {
                                MAY_UPD_IMP_COL();
                                scanner->rlt_sch = RS_STR;
                                if (valid_symbols[R_SGL_PLN_STR_BLK] && is_r)
                                    RET_SYM(R_SGL_PLN_STR_BLK);
                                if (valid_symbols[BR_SGL_PLN_STR_BLK] && is_br)
                                    RET_SYM(BR_SGL_PLN_STR_BLK);
                                if (valid_symbols[B_SGL_PLN_STR_BLK] && is_b)
                                    RET_SYM(B_SGL_PLN_STR_BLK);
                                if (valid_symbols[R_SGL_PLN_STR_FLW] && is_r)
                                    RET_SYM(R_SGL_PLN_STR_FLW);
                                if (valid_symbols[BR_SGL_PLN_STR_FLW] && (is_br || has_nwl))
                                    RET_SYM(BR_SGL_PLN_STR_FLW);
                            }
                            ...
                            if (!can_hlm_tpl && !is_hlm_control && !is_hlm_blk_comment) {
                                ...
                                mrk_end(scanner, lexer);
                                if (lexer->lookahead == ':') {
                                    if (hlm_first_expr_chr == '$' && can_comment) {
                                        while (lexer->lookahead != 0 && !is_nwl(lexer->lookahead)) {
                                            adv(scanner, lexer);
                                        }
                                        mrk_end(scanner, lexer);
                            ...
                            if (is_hlm_blk_comment && can_comment)
                                RET_SYM(COMMENT);

                            if (is_hlm_control) {
                                if (is_hlm_else && is_line_start) {
                                    scanner->skip_empty_map_after_else = 1;
                                    scanner->skip_empty_map_after_else_col = bgn_col;
                                }
                                if (prefer_hlm_dir && can_hlm_dir) {
                                    if (is_r_hlm_dir)
                                        RET_SYM(R_HLM_DIR);
                                    if (is_br_hlm_dir)
                                        RET_SYM(BR_HLM_DIR);
                            ...
                            if (!can_hlm_tpl && !can_hlm_dir && can_comment)
                                RET_SYM(COMMENT);
                ...
                if (!can_hlm_tpl && !is_hlm_control && !is_hlm_blk_comment) {
                    for (;;) {
                        if (lexer->lookahead == 0 || is_wht(lexer->lookahead) || is_nwl(lexer->lookahead) || lexer->lookahead ==
                                                                                           '#' ||
        ...
        bool is_r_flw_jsv_bgn = valid_symbols[R_FLW_JSV_BGN] && is_r;
        bool is_br_flw_jsv_bgn = valid_symbols[BR_FLW_JSV_BGN] && is_br;
        ...
        if (is_r_flw_jsv_bgn || is_br_flw_jsv_bgn || is_r_blk_val_bgn || is_br_blk_val_bgn || is_b_blk_val_bgn ||
            is_r_blk_imp_bgn || is_r_flw_njv_bgn || is_br_flw_njv_bgn) {
            adv(scanner, lexer);
        }
